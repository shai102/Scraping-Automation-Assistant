...
def bg_update_single_ui(gui, idx, title, t_id, msg, meta):
    ...
    try:
        ...
        # Extract episode number from the current filename first. In batch
        # sync, metadata may come from another selected row, so reusing m["e"]
        # would rename sibling files to the same episode.
...
def process_task(gui, i, advance_progress=True):
    ...
    try:
        ...
        _folder_id_for_cache = (
            extract_db_id_from_path(item.path, mode, _folder_id_title_hints) or ""
        )
            ...
            if is_resolver:
                try:
                    folder_id = extract_db_id_from_path(
                        item.path, mode, _folder_id_title_hints
                    )
                    if folder_id:
        ...
        gui.root.after(
            ...
            lambda: gui.tree.item(
                ...
                tid,
                item.full_target or new_fn,
                gui._build_status_text(ai_msg, recap_status, db_m),
            ),
        ),
        )
