...
适用范围：后续所有新会话，只要用户提到 GUI 版 / WEB 版发版、打 tag、创建 Release、上传发行资产，都优先按本文件执行。
...
4. 推送当前分支到远端。
5. GUI 版如需发安装包，执行项目内现成的打包脚本并生成 exe / `.rar`；WEB 版跳过这一步。
6. 创建对应版本 tag，tag 名直接用版本号，例如 `v4.8`。
7. 推送 tag 到远端。
8. 创建 GitHub Release。
9. WEB 版默认不上传 Windows EXE / `.rar`；如用户特别要求，再按实际产物上传对应资产。
10. 最后再核对 Release 页面、tag、版本号、正文内容是否一致。
11. 如果本次额外生成了临时发行包，发布完成后删除仓库根目录里的临时产物，避免本地残留。

## 用户偏好的执行方式

- 用户通常希望我直接把整套流程做完：`commit -> push -> tag -> push tag -> release`。
- 如果用户说“继续发布流程吧”，默认表示：
  - 不需要重新改代码
  - 不需要重新分析需求
  - 直接从发布链路剩余步骤继续
- 如果本地产物已经准备好，优先从 `git add` / `commit` / `push` 之后的断点续做，不重复构建或打包。
...
- GUI 版资产名：`vX.Y-windows-exe.rar`
- WEB 版默认无发行资产；若用户特别要求附件，按实际产物命名并上传。
...
- 如果上传资产，先确认附件来自最新构建，不要误传旧包。
- 新建 Release 后再次核对版本号、正文和附件名是否准确。
- 如果本次上传了临时生成的发行包，核对完成后默认删除本地仓库根目录里的对应临时文件。
...
默认先核对仓库路径、版本号、是否需要发行资产，再进入发布步骤。
