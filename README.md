...
- **删除同步**：在监控目录删除文件或文件夹时，自动同步删除导出目标目录中对应的软链接/复制文件及刮削整理后的目标文件（支持两跳链路：软链接目录 → 刮削整理目录）。作品最后一个媒体文件被删除后，会一并删除作品目录内的 NFO、海报等全部元数据和整个作品目录；仍有其他集数或媒体文件时则保留共享元数据。DB 记录被清空时仍可通过 `target_root + 相对路径` 对第一跳进行兜底删除；各整理模式（copy / symlink / hardlink）同样支持删除同步，move / rename 模式不受影响。
