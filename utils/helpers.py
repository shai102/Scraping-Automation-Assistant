...
_DECIMAL_EP_RE = re.compile(
    r"""(?ix)
    (?:
        s\d{1,2}\s*e\d{1,4}\.\d(?!\d)          # S01E4.5
        | (?:ep?)\s*0*\d{1,4}\.\d(?!\d)          # E4.5, EP04.5
        | \u7b2c\s*0*\d{1,4}\.\d(?!\d)\s*[\u96c6\u8bdd\u8a71]  # \u7b2c4.5\u96c6
        | [\[\(\uff08]\s*0*\d{1,4}\.\d(?!\d)\s*[\]\)\uff09]   # [4.5] (4.5)
        | (?<![.\d])-\s*0*\d{1,2}\.\d(?!\d)(?=[\s\[\(\uff08]|$)  # - 4.5
    )"""
)
...
def build_db_query_plan(item, query_title, ai_data, g):
    """Build staged DB query titles for headless matching.
...
# ---------------------------------------------------------------------------
# Metadata completeness check — used by the periodic refresh patrol
# ---------------------------------------------------------------------------
